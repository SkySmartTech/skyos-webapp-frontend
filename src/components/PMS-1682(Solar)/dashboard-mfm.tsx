...
const rtData = [
  { label: "Thermal Energy",       value: "960 KW",    color: "text-[#14B8A6]" },
  { label: "CH Water Flow",        value: "264 m³/h",  color: "text-[#14B8A6]" },
  { label: "CH Water Supply Temp", value: "8.0 °C",    color: "text-[#22C55E]" },
  { label: "CH Water Return Temp", value: "11.1 °C",   color: "text-[#f59e0b]" },
  { label: "Total kW",             value: "1,572 kW",  color: "text-[#14B8A6]" },
  { label: "Chiller COP",          value: "3.2",       color: "text-[#22C55E]" },
  { label: "System Load",          value: "80 %",      color: "text-[#f59e0b]" },
...
export default function HVACMFMDashboard() {
  ...
  const kpis = [
    { label: "MFM kW",      value: "960",  unit: "kW",   icon: Zap,         color: dark ? "bg-teal-600/10 border-teal-500/20 text-teal-400"    : "bg-teal-50 border-teal-200 text-teal-600",      trend: "+2.1%" },
    { label: "Water Flow",  value: "264",  unit: "m³/h", icon: Droplets,    color: dark ? "bg-teal-600/10 border-teal-500/20 text-teal-400"    : "bg-teal-50 border-teal-200 text-teal-600",      trend: "Normal" },
    { label: "Supply Temp", value: "8.0",  unit: "°C",   icon: Thermometer, color: dark ? "bg-green-600/10 border-green-500/20 text-green-400" : "bg-[#f0fdf4] border-[#dcfce7] text-[#22C55E]",  trend: "Optimal" },
    { label: "Return Temp", value: "11.1", unit: "°C",   icon: Thermometer, color: dark ? "bg-teal-600/10 border-teal-500/20 text-teal-400"    : "bg-teal-50 border-teal-200 text-teal-600",      trend: "Stable" },
  ];
    ...
    <div className={`font-sans flex flex-col h-full p-3 sm:p-4 md:p-5 gap-3 md:gap-4 transition-colors duration-300 ${bg}`}>
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
        ...
        <div className={`border rounded-xl p-4 flex flex-col transition-colors duration-300 ${card}`}>
          <p className={`text-xs font-bold uppercase tracking-widest mb-3 text-center ${textMut}`}>MFM Unit — Real-Time</p>
          ...
          <div className="flex flex-col items-center mb-3">
            ...
            <p className={`text-4xl font-black mt-1 ${textPri}`}>960 <span className={`text-sm font-normal ${textSec}`}>kW</span></p>
          </div>
      ...
      <div className={`pt-3 border-t flex flex-wrap items-center justify-between gap-2 text-[11px] ${divider} ${textMut}`}>
        <span>MFM Unit — Main Factory Manufacturing</span>
        <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-[#22C55E]" /> MFM Online</span>
      </div>
    </div>
  );
}
