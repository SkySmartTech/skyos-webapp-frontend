...
const totalDowntime = departmentCards.reduce(
...
export default function AndonHomeDashboard() {
  ...
  const pieData = departmentCards.map((item) => ({
    name: item.name,
  ...
  return (
    <div
      className={`min-h-full w-full overflow-y-auto p-4 md:p-6 transition-colors duration-300 ${bg}`}
    >
      <div
        className={`mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border px-4 py-3 shadow-sm ${card}`}
      >
        <div>
          <p
            className={`text-xs font-semibold uppercase tracking-[0.28em] ${muted}`}
          >
            Andon Home Dashboard
          </p>
          <h1 className={`text-2xl font-black tracking-tight ${pageText}`}>
            Downtime Overview
          </h1>
        </div>
        <div
          className={`rounded-xl border px-4 py-2 text-sm font-semibold ${dark ? "border-gray-800 bg-gray-950 text-gray-200" : "border-gray-200 bg-slate-50 text-gray-700"}`}
        >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[340px_1.1fr_1.35fr] gap-4">
        <section
          className={`overflow-hidden rounded-xl border shadow-sm ${card}`}
        >
          <div className="bg-cyan-600 px-5 py-4 text-white">
            <div className="text-4xl font-black tracking-tight">
              TOTAL :{formatValue(totalDowntime)}
            </div>
          </div>
        ...
        >
          <div className="bg-rose-600 px-5 py-4 text-white">
            <h2 className="text-xl font-semibold">
              Department Wise Total Downtime
            </h2>
          </div>
          <div className="p-4">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Legend
                    ...
                    formatter={(value) => (
                      <span
                        style={{ color: axis, fontSize: 12, fontWeight: 600 }}
                      >
                        {value}
                      </span>
                    )}
          ...
          >
            <div className="bg-emerald-600 px-5 py-4 text-white">
              <h2 className="text-lg font-semibold">
                Daily Downtime : {item.name}
              </h2>
            </div>
            <div className="p-4">
              <div
                className={`h-60 rounded-lg border p-2 ${chartBg} ${dark ? "border-gray-800" : "border-gray-100"}`}
              >
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart
                    data={dailyDowntime.map((entry) => ({
                      day: entry.day,
