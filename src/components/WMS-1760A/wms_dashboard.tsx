...
import BuildingMaintenanceReport from "./building_maintenance_report";
import OtherReport from "./other_report";
...
function Placeholder({ title }: { title: string }) {
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center h-full gap-4 ${dark ? "text-gray-400" : "text-gray-500"}`}
    >
      <div
        className={`w-16 h-16 rounded-2xl flex items-center justify-center ${dark ? "bg-[#1e293b]" : "bg-gray-100"}`}
      >
...
export default function WmsDashboard({
  ...
  if (activeSection === "report-building") return <BuildingMaintenanceReport />;
  if (activeSection === "report-other") return <OtherReport />;
  return <WmsHome />;
}
