...
const TABLE_DATA = [
  { dept: "Engineering", machineCat: "JL", machineNo: "JL 03", status: "Completed", scheduledDate: "2026-06-15", completedDate: "2026-06-16", techName: "Francis", duration: "02:30" },
  { dept: "Knitting", machineCat: "JL", machineNo: "JL 01", status: "Completed", scheduledDate: "2026-06-14", completedDate: "2026-06-14", techName: "Aravinda", duration: "01:45" },
  { dept: "DryFinishing", machineCat: "Rewinding", machineNo: "RW 01", status: "Pending", scheduledDate: "2026-06-19", completedDate: "—", techName: "Janaka", duration: "—" },
  { dept: "Dyeing", machineCat: "Jet dyeing m.", machineNo: "JD 05", status: "Completed", scheduledDate: "2026-06-13", completedDate: "2026-06-13", techName: "Suranga", duration: "03:00" },
  { dept: "Finishing", machineCat: "Steamer", machineNo: "ST 02", status: "Completed", scheduledDate: "2026-06-17", completedDate: "2026-06-17", techName: "Ruwan", duration: "01:20" },
];

const DEPARTMENTS = ["All", "Engineering", "Finishing", "Knitting", "DryFinishing", "Dyeing"];
const MACHINE_CATEGORIES = ["All", "JL", "SC", "SB", "TX", "MJ", "Jet dyeing m.", "Steamer"];
    ...
    <div className={`flex flex-col h-full overflow-auto ${bg}`}>
      <div className="p-4 space-y-4">
        ...
        <div className={`rounded-xl border p-4 ${card}`}>
          <h2 className={`text-base font-bold mb-4 ${text}`}>Planned Maintenance</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className={`rounded-xl border p-4 ${card}`}>
            ...
            <ResponsiveContainer width="100%" height={180}>
              <PieChart>
                <Pie data={DEPT_PIE} dataKey="value" cx="50%" cy="50%" outerRadius={70} label={({ value }) => value}>
          ...
          {/* Status Donut */}
          <div className={`rounded-xl border p-4 ${card}`}>
            <p className={`text-sm font-bold mb-3 ${text}`}>Planned Maintenance Status</p>
            ...
            <ResponsiveContainer width="100%" height={180}>
              <PieChart>
                <Pie data={STATUS_DONUT} dataKey="value" cx="50%" cy="50%" innerRadius={45} outerRadius={70} label={({ value }) => value}>
          ...
          <div className={`rounded-xl border p-4 ${card}`}>
            <p className={`text-sm font-bold mb-3 ${text}`}>Machine Category Wise Planned Maintenance</p>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={MACHINE_CAT_BAR} margin={{ top: 5, right: 5, left: -25, bottom: 40 }}>
                <CartesianGrid strokeDasharray="3 3" stroke={dark ? "#1e293b" : "#f0f0f0"} />
                <XAxis dataKey="cat" tick={{ fontSize: 7, fill: dark ? "#64748b" : "#9ca3af" }} angle={-45} textAnchor="end" />
        ...
        <div className={`rounded-xl border p-4 ${card}`}>
          <p className={`text-sm font-bold mb-2 ${text}`}>Machine Wise Maintenance</p>
        ...
        <div className={`rounded-xl border overflow-hidden ${card}`}>
          <div className="bg-red-600 px-4 py-2.5">
            <h3 className="text-sm font-bold text-white">Planned Maintenance W/O Details</h3>
          </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {filteredTable.map((row, i) => (
                  <tr key={i} className={`border-t ${tblRow}`}>
                    ...
                    <td className="px-3 py-2">
                      <span className={`px-2 py-0.5 rounded text-xs font-semibold ${row.status === "Completed" ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"}`}>
                        {row.status}
                      </span>
                    </td>
                ...
                {filteredTable.length === 0 && (
                  <tr><td colSpan={8} className={`text-center py-6 text-xs ${muted}`}>No records found</td></tr>
                )}
              </tbody>
            </table>
          </div>
