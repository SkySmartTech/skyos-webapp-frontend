    ...
    <div className={`h-full w-full p-6 ${page}`}>
      ...
      <div className="mb-4 inline-flex rounded-2xl border p-1 shadow-sm shadow-black/5 dark:border-white/10 dark:bg-white/5">
        <button
          ...
          className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${activeTab === "roles" ? "bg-zinc-800 text-white dark:bg-white dark:text-slate-900" : `${muted} hover:text-white dark:hover:text-white`}`}
        >
          Roles
        </button>
        <button
          onClick={() => setActiveTab("permissions")}
          className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${activeTab === "permissions" ? "bg-zinc-800 text-white dark:bg-white dark:text-slate-900" : `${muted} hover:text-white dark:hover:text-white`}`}
        >
          Permissions
        </button>
      </div>
          ...
          {roleCards.map((role) => (
            ...
            >
              ...
              <div className="mt-6">
                <button
                  onClick={() => setShowModal(true)}
                  className={`w-full rounded-xl border py-2.5 text-sm font-semibold transition ${dark ? "border-white/10 bg-[#111111] text-white hover:bg-white/5" : "border-slate-200 bg-slate-50 text-slate-900 hover:bg-slate-100"}`}
                >
                  Manage Permissions
                </button>
              </div>
            ...
            <div className="mt-5 space-y-4">
              ...
              <label className="block">
                ...
                <select
                  value={form.level}
                  onChange={(event) =>
                    setForm((c) => ({ ...c, level: event.target.value }))
                  }
                  className={`w-full rounded-xl border px-4 py-3 text-sm outline-none ring-0 ${field}`}
                >
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            setForm((c) => ({ ...c, [checkedKey]: !checked }))
                          }
                          className={`relative h-6 w-11 rounded-full border transition ${checked ? "border-orange-500 bg-orange-500" : "border-slate-300 bg-slate-200"}`}
                        >
                          <span
