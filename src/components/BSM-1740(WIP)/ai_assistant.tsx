  ...
  function send() {
    ...
    setTimeout(() => {
      const reply: Message = {
        id: Date.now() + 1,
        from: "ai",
        text: "I can help with reports, inventory queries, or trend analysis. What would you like to do?",
      };
          ...
          <div className="flex-1 flex flex-col">
            <div ref={scroller} className="flex-1 overflow-y-auto p-6">
              {messages.map((m) => (
                ...
                >
                  <div
                    className={`max-w-[72%] rounded-xl px-4 py-3 text-sm ${m.from === "user" ? "bg-orange-500 text-white" : "bg-white/5 text-slate-200 border border-white/5"}`}
                  >
                    {m.text}
                  </div>
                </div>
            ...
            <div className="border-t p-4">
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="flex-1 rounded-xl border px-4 py-3 bg-transparent text-sm outline-none"
                />
                <button
                  onClick={send}
                  className="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-orange-500 text-white"
                  aria-label="Send"
                >
                  <Send size={16} />
                </button>
