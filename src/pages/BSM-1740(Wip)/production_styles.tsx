...
export default function ProductionStyles() {
  ...
  function addStyle(e?: React.FormEvent) {
    ...
    setStyles((s) => [
      {
        id,
        code: form.code || `STY-${String(id).slice(-3)}`,
        name: form.name,
        type: form.type,
        color: form.color,
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-black">
            Style Configurations (10 Examples)
          </h1>
          <p className="mt-1 text-sm text-slate-500">
            Manage style codes, materials and status
          </p>
        </div>
        <button
            ...
            {styles.map((s) => (
              <tr key={s.id} className="border-t border-white/5">
                ...
                <td className="px-6 py-4">
                  <span
                    className={`rounded-full px-3 py-1 text-xs ${s.status === "Active" ? "bg-white/5 text-white" : "bg-slate-700 text-white"}`}
                  >
                    {s.status}
                  </span>
                </td>
